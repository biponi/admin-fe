...
const SalesTab: React.FC<Props> = ({ data, formatCurrency, formatNumber }) => {
  ...
  const comparison = data.comparison || {
    ...
    previousPeriod: {} as any,
    growth: { orders: 0, revenue: 0, aov: 0 },
  };
  const growthCards = [
    {
      label: "Orders Growth",
      value: comparison.growth?.orders || 0,
      positive: (comparison.growth?.orders || 0) >= 0,
    },
    {
      label: "Revenue Growth",
      value: comparison.growth?.revenue || 0,
      positive: (comparison.growth?.revenue || 0) >= 0,
    },
    {
      label: "AOV Growth",
      value: comparison.growth?.aov || 0,
      positive: (comparison.growth?.aov || 0) >= 0,
    },
  ];
    ...
    datasets: [
      {
        ...
        backgroundColor: CHART_COLORS.slice(0, brandData.length).map(
          (c) => c + "cc",
        ),
      ...
      <div className='grid grid-cols-2 md:grid-cols-4 gap-4'>
        {growthCards.map((card, i) => (
          <div
            ...
            <p
              className={`text-xl font-bold ${card.positive ? "text-emerald-600" : "text-red-600"}`}>
              {card.positive ? "+" : ""}
        ...
        <div className='bg-white rounded-xl border border-slate-100 shadow-sm p-4'>
          ...
          <p className='text-xl font-bold text-slate-900'>
            {formatCurrency(comparison.currentPeriod?.aov || 0)}
          </p>
        </div>
      </div>
      ...
      <Card className='border-slate-100 shadow-sm'>
        <CardHeader className='pb-2 flex flex-row items-center justify-between'>
          ...
          <InfoPopover
            ...
            description='Tracks your daily revenue, discounts given, and profit over time. Includes all orders regardless of status.'
            formula='Gross Revenue = Total order value before discounts | Net Revenue = After discounts | Discounts = Amount reduced | AOV = Net revenue divided by number of orders'
          />
        </CardHeader>
        <CardContent>
                      ...
                      label: (ctx: any) =>
                        ctx.dataset.type === "line"
                          ...
                          : ctx.dataset.type === "bar" &&
                              ctx.dataset.label === "Discounts"
                            ? ` Discounts: ${formatCurrency(ctx.raw)}`
                            : ` ${ctx.dataset.label}: ${formatCurrency(ctx.raw)}`,
                    },
                  },
                },
        ...
        <Card className='border-slate-100 shadow-sm'>
          <CardHeader className='pb-2 flex flex-row items-center justify-between'>
            <div>
              ...
              <p className='text-xs text-slate-500'>
                Revenue and order breakdown by category
              </p>
            </div>
            <InfoPopover
              title='Sales by Category'
              description='Net revenue and number of orders for each product category.'
              formula='Revenue = Sales value after discounts for products in each category | Orders = Distinct orders containing products from that category. Sorted by revenue.'
            />
          </CardHeader>
          <CardContent>
        ...
        <Card className='border-slate-100 shadow-sm'>
          <CardHeader className='pb-2 flex flex-row items-center justify-between'>
            ...
            <InfoPopover
              ...
              description='Total revenue generated by each brand from product sales.'
              formula='Revenue = Sum of (product price minus discount) for all items belonging to each brand. Sorted by highest revenue.'
            />
          </CardHeader>
          <CardContent>
            <div style={{ height: 300 }}>
              <ChartComponents.Bar
                ...
                options={{
                  ...
                  scales: {
                    ...
                    y: { ...defaultScaleOptions.y, grid: { display: false } },
                  },
                }}
              />
            </div>
      ...
      <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
        <Card className='border-slate-100 shadow-sm'>
          <CardHeader className='pb-2 flex flex-row items-center justify-between'>
            ...
            <InfoPopover
              ...
              description='How much revenue came through each payment method (COD, Bkash, Nagad, Card, etc.).'
              formula='Revenue = Total amount collected per payment method | Count = Number of transactions | Average = Revenue divided by transactions'
            />
          </CardHeader>
          <CardContent>
            <div style={{ height: 300 }}>
        ...
        <Card className='border-slate-100 shadow-sm'>
          <CardHeader className='pb-2 flex flex-row items-center justify-between'>
            ...
            <InfoPopover
              title='Sales Channels (Radar)'
              description='Revenue and order count from each sales channel (website, app, marketplace, etc.).'
              formula='Revenue = Net sales (after discounts) per channel | Orders = Number of orders placed through each channel'
            />
          </CardHeader>
          <CardContent>
            <div style={{ height: 300 }}>
