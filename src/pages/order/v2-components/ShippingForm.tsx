...
export function ShippingForm({ shipping, onChange, onDeliveryChargeChange }: ShippingFormProps) {
      ...
      <CardContent className="p-3 space-y-4">
        <div className="space-y-3.5">
          ...
          <div className="space-y-1.5">
            <Label htmlFor="division" className="flex items-center gap-1.5 font-medium text-sm">
            ...
            >
              <SelectTrigger id="division" className="h-9 border-gray-200 focus:border-green-500 transition-all">
              ...
              <SelectContent className="max-h-64">
                ...
                <div className="max-h-48 overflow-y-auto">
                  {filteredDivisions.map((division) => (
                    <SelectItem key={division.id} value={division.name} className="text-sm">
                      {division.name}
                    </SelectItem>
                  ))}
                </div>
          ...
          {/* District */}
          <div className="space-y-1.5">
            <Label htmlFor="district" className="flex items-center gap-1.5 font-medium text-sm">
            ...
            <Select
              ...
              onValueChange={(value) => {
                ...
                if (shipping.division && value && onDeliveryChargeChange) {
                  const chargeInfo = calculateDeliveryCharge(value, shipping.division);
                  onDeliveryChargeChange(chargeInfo.charge);
                }
              }}
              disabled={!selectedDivision}
            >
              <SelectTrigger id="district" className="h-9 border-gray-200 focus:border-green-500 transition-all">
              ...
              <SelectContent className="max-h-64">
                ...
                <div className="max-h-48 overflow-y-auto">
                  {filteredDistricts.map((district) => (
                    <SelectItem key={district.id} value={district.name} className="text-sm hover:bg-green-50">
                      {district.name}
                    </SelectItem>
                  ))}
                </div>
          ...
          {shipping.division && shipping.district && (
            <div className="mt-4 p-3 bg-green-50 rounded-lg border border-green-200">
              ...
              <div className="text-sm text-green-700 mt-2 space-y-0.5">
                <p><strong>Division:</strong> {shipping.division}</p>
          ...
          {/* Address */}
          <div className="space-y-1.5">
            <Label htmlFor="address" className="flex items-center gap-1.5 font-medium text-sm">
              <MapPin className="w-3.5 h-3.5 text-green-600" />
              Full Address *
            </Label>
            <Textarea
