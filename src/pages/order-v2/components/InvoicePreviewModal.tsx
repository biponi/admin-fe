 ...
 * Unified preview for multiple invoices with navigation and print capabilities
 */
...
export const InvoicePreviewModal: React.FC<InvoicePreviewModalProps> = ({
  ...
  const handlePrintAll = async () => {
    try {
      ...
      // Create a new PDF document to merge all invoices
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className='max-w-[95vw] w-[95vw] h-[95vh] p-0 gap-0'>
          ...
          <div className='flex items-center gap-1.5'>
            ...
            {pdfUrls.length > 1 && (
              <Button
                ...
                className='h-7 px-2 text-xs'
                title={`Print all ${pdfUrls.length} invoices`}>
                <Printer className='h-3.5 w-3.5 mr-1.5' />
                All ({pdfUrls.length})
              </Button>
            )}
          ...
          <AnimatePresence mode='wait'>
            <motion.div
              ...
              <iframe
                ...
                title={`Invoice ${currentPdf?.orderNumber}`}
        ...
        {/* Minimal Footer Navigation - Only show if multiple invoices */}
        {pdfUrls.length > 1 && (
